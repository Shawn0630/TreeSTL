 ...
 * @since 1.0.0
 */
public abstract class TreeNodeImplementation<T> extends TreeNode<T> {
     ...
     */
    public TreeNodeImplementation(T data) {
        super(data);
    }
     ...
     */
    public boolean addSubtrees(Collection<? extends TreeNodeImplementation<T>> subtrees) {
        if (areAllNulls(subtrees)) {
            return false;
        }
        for (TreeNodeImplementation<T> subtree : subtrees) {
     ...
     *         no subtrees
     */
    public Collection<? extends TreeNodeImplementation<T>> siblings() {
        if (isRoot()) {
        ...
        Collection<TreeNodeImplementation<T>> siblings = new HashSet<>(parentSubtreesSize - 1);
        for (TreeNode<T> parentSubtree : parentSubtrees) {
            if (!parentSubtree.equals(this)) {
                siblings.add((TreeNodeImplementation<T>) parentSubtree);
            }
        }
        return siblings;
    }
     ...
     */
    public boolean hasSubtrees(Collection<? extends TreeNodeImplementation<T>> subtrees) {
        if (isLeaf()
     ...
     */
    public boolean dropSubtrees(Collection<? extends TreeNodeImplementation<T>> subtrees) {
        if (isLeaf()
                || areAllNulls(subtrees)) {
            return false;
        }
